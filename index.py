...
if isInternetConnected == True:
    ...
    sp = spotipy.Spotify(
        auth_manager=SpotifyOAuth(
            client_id=spotifyApiKeyClientId,
            client_secret=spotifyApiKeyClientSecret,
    ...
    connectionErrorHeading = tk.Label(
        frameForError,
    ...
    connectionErrorText = tk.Label(
        frameForError,
        text="you are not connected to the internet, please do and restart the app",
        font='"Space Grotesk" 13',
        anchor="w",
        bg="#FFFFFF",
        fg="#000000",
...
def askForImage():
    ...
    while True:
        getImage = filedialog.askopenfilenames(
            title="select artwork", filetypes=(("png", "*.png"), ("jpg", "*.jpg"))
        )
        if getImage:
            break
...
def askForSong():
    global getSong
    while True:
        getSong = filedialog.askopenfilenames(
            title="select song", filetypes=(("mp3", "*.mp3"), ("wav", "*.wav"))
        )
        if getSong:
            break
    global songLocation
    ...
    shortenedLocation = (
        songLocation[:maxLengthOfText] + "..."
        if len(songLocation) > maxLengthOfText
        else songLocation
    )
        ...
        if isSongButtonClicked == False:
            ...
            if error == True:
                messagebox.showwarning(
                    "internal error", "update default values for text"
                )
                ...
                if not miscFilesText.startswith("https://"):
                    messagebox.showwarning(
                        "internal error",
                        "misc files should be uploaded to a cloud drive and the web address should be stored",
                    )
                    ...
                    for image in getImage:
                        ...
                        cursor.execute(
                            dataInsertQuery,
                            (imageLocation, songLocation, miscFilesText, insertPhoto),
                        )
                    conn.commit()
                    conn.close()
                    ...
                    addMiscFiles.insert(0, "link to misc files")
                    importArtworkImage.config(
                        text="import artwork", image="", bg="#FFFFFF"
                    )
                    songLocation = None
                    isSongButtonClicked = False
                    isImageButtonClicked = False
...
def addValuesToDB():
    ...
    def addTrackingForEntry(recievedData):
        ...
        backButtonTracking = tk.Button(
            ...
            activebackground="#FFFFFF",
            borderwidth=0,
            command=lambda: [showPage(frame), addValuesToDB()],
            cursor="hand2",
            justify=tk.LEFT,
        )
        backButtonTracking.grid(row=0, column=0, pady=30, sticky="w")

        songTitleTracking = tk.Label(
            frameForTracking,
            text=recievedData,
            relief="flat",
            font='"Space Grotesk" 13',
        ...
        trackingSection = tk.Label(
            frameForTracking, bg="#FFFFFF", activebackground="#FFFFFF"
        )

        trackYoutubeText = tk.Label(
            trackingSection,
            text="start tracking for Youtube",
            relief="flat",
            font='"Space Grotesk" 13',
            width=0,
            fg="#B0B0B0",
        ...
        trackYoutubeEntry.grid(
            row=0, column=1, sticky="nsew", padx=(0, 50), pady=(10, 0)
        )

        trackSpotifyText = tk.Label(
            trackingSection,
            text="                                      Spotify",
            relief="flat",
            font='"Space Grotesk" 13',
        ...
        trackSpotifyEntry.grid(
            row=1, column=1, sticky="nsew", padx=(0, 50), pady=(10, 0)
        )

        saveTrackingButton = tk.Button(
            trackingSection,
            font='"Space Grotesk" 13',
            bg="#FFFFFF",
            text="start tracking",
            ...
            borderwidth=0,
            cursor="hand2",
            command=lambda: saveTracking(
                recievedData, trackYoutubeEntry, trackSpotifyEntry
            ),
        )
        saveTrackingButton.grid(
            row=2, column=1, pady=(155, 0), padx=(0, 44), sticky="e"
        )
    ...
    if numberOfEntries == 0:
        noReleasesText = tk.Label(
            previousReleases,
            text="there are no releases at the moment",
            font='"Space Grotesk" 13',
            anchor="w",
            padx=20,
            bg="#FFFFFF",
        ...
        for i in range(numberOfEntries):
            ...
            def deleteEntryWithAnalytics(recievedData):
                deleteReleaseQuestion = tk.messagebox.askquestion(
                    "delete release",
                    "you sure you want to delete this release?",
                    icon="warning",
                )
                if deleteReleaseQuestion == "yes":
                    ...
                    deleteValuesFromTable = (
                        """DELETE FROM releaseData WHERE songTitle = ?"""
                    )
            ...
            if youtubeLinkFetch != None:
                ...
                entry = tk.Button(
                    ...
                    width=62,
                    cursor="hand2",
                    command=createCommandForOpen(fetchEntry),
                )
                ...
                youtubeRequest = youtubeConnect.videos().list(
                    part="statistics", id=youtubeVideoId
                )
                youtubeResponse = youtubeRequest.execute()
                youtubeViewCount = youtubeResponse["items"][0]["statistics"][
                    "viewCount"
                ]
                ...
                youtubeAnalytics = tk.Button(
                    ...
                    relief="flat",
                    foreground="#FE0404",
                    text=youtubeViewCount,
                    command=createCommandForYoutubeButton(youtubeLinkFetch),
                    cursor="hand2",
                    width=5,
                )
                youtubeAnalytics.grid(row=0, column=0, pady=(0, 3))
                spotifyAnalytics = tk.Button(
                    addTrackingButton,
                    font='"Space Grotesk" 11',
                    anchor="w",
                    bg="#FFFFFF",
                    ...
                    relief="flat",
                    foreground="#1DD05D",
                    text=spotifySongPopularity,
                    command=createCommandForSpotifyButton(spotifyLinkFetch),
                    cursor="hand2",
                    width=3,
                )
                spotifyAnalytics.grid(row=0, column=1, pady=(0, 3), padx=(3, 0))
                addTrackingButton.grid(
                    row=i + 1,
                    column=1,
                    sticky="w",
                    columnspan=2,
                ...
                deleteButton = tk.Button(
                    previousReleases,
                    image=resizedDeleteImagePhotoImage,
                    cursor="hand2",
                    relief="solid",
                    borderwidth=1,
                    background="#FFFFFF",
                    activebackground="#FFFFFF",
                    command=createCommandForDeleteWithAnalytics(fetchEntry),
                    width=29,
                    height=29,
                )
                deleteButton.image = resizedDeleteImagePhotoImage
                deleteButton.grid(
                    row=i + 1, column=3, sticky="w", padx=(20, 15), pady=(0, 3)
                )
                ...
                def saveTracking(recievedData, trackYoutubeEntry, trackSpotifyEntry):
                    ...
                    if spotifyLinkValue == "":
                        errorTracking = True
                    if not youtubeLinkValue.startswith(
                        "https://www.youtube.com/watch?v"
                    ):
                        errorTracking = True
                    if not spotifyLinkValue.startswith(
                    ...
                    if errorTracking == True:
                        ...
                        cursor.execute(
                            addTrackingToTable,
                            (youtubeLinkValue, spotifyLinkValue, recievedData),
                        )
                        conn.commit()
                        cursor.execute(
                            "SELECT youtubeLink,spotifyLink from releaseData"
                        )
                        fetchTrackingLinks = cursor.fetchall()
                        conn.commit()
                        conn.close()
                ...
                addTrackingButton = tk.Button(
                    ...
                    cursor="hand2",
                    relief="solid",
                    command=createCommandForTracking(fetchEntry),
                    activebackground="#FFFFFF",
                )
                addTrackingButton.grid(
                    row=i + 1,
                ...
                deleteButton = tk.Button(
                    ...
                    background="#FFFFFF",
                    activebackground="#FFFFFF",
                    command=createCommandForDeleteWithoutAnalytics(fetchEntry),
                    width=29,
                    height=29,
                )
                deleteButton.image = resizedDeleteImagePhotoImage
                deleteButton.grid(
                    row=i + 1, column=3, sticky="w", padx=(20, 10), pady=(0, 3)
                )
    ...
    viewAnalytics = tk.Button(
        ...
        padx=20,
        cursor="hand2",
        command=lambda: showPage(frameForAnalytics),
    )
    ...
    if numberOfEntriesWithAnalytics == 0:
        ...
        noAnalyticsErrorHeading = tk.Label(
            frameForErrorNoAnalytics,
        ...
        noAnalyticsErrorText = tk.Label(
            frameForErrorNoAnalytics,
            text="tracking hasn't been added to any entries, please do and check back later",
            font='"Space Grotesk" 13',
            anchor="w",
            bg="#FFFFFF",
            fg="#000000",
        ...
        for j in range(len(fetchDataForAnalysis)):
            ...
            youtubeVideoId = youtubeLinkForAnalysis[
                youtubeVideoId : youtubeVideoId + 11
            ]
            youtubeRequest = youtubeConnect.videos().list(
                part="statistics", id=youtubeVideoId
            )
            ...
            spotifyTrackId = spotifyLinkForAnalysis[
                spotifyTrackId : spotifyTrackId + 22
            ]
            spotifyTrackInfo = sp.track(spotifyTrackId)
            spotifySongPopularity = spotifyTrackInfo["popularity"]
            spotifySongPopularity = int(spotifySongPopularity)
            ...
            analyticsDataForGraphs.append(
                [
                    songTitleForAnalysis,
                    youtubeViewCount,
                    spotifySongPopularity,
                    popTagForAnalysis,
                    hiphopTagForAnalysis,
                    indieTagForAnalysis,
                    kpopTagForAnalysis,
                    explicitTagForAnalysis,
                    inhouseTagForAnalysis,
                    lofiTagForAnalysis,
                ]
            )
        ...
        avgYoutubeViewsText = tk.Label(
            frameForAnalytics,
            font='"Space Grotesk" 13',
            fg="#B0B0B0",
            bg="#FFFFFF",
            text="avg youtube views",
            anchor="sw",
            padx=5,
            pady=5,
            width=18,
        ...
        avgSpotifyPopularityNumber = int(
            totalSpotifyCount / numberOfEntriesWithAnalytics
        )

        avgYoutubeViews = tk.Label(
            frameForAnalytics,
            font='"Space Grotesk" 21',
            width=8,
            fg="#000000",
            bg="#FFFFFF",
            text=avgYoutubeViewsNumberWithCommas,
            anchor="e",
            padx=5,
            pady=5,
        )
        ...
        avgSpotifyPopularityText = tk.Label(
            frameForAnalytics,
            font='"Space Grotesk" 13',
            fg="#B0B0B0",
            bg="#FFFFFF",
            text="avg spotify popularity",
            anchor="sw",
            padx=5,
            pady=5,
            width=18,
        ...
        avgSpotifyPopularity = tk.Label(
            frameForAnalytics,
            font='"Space Grotesk" 21',
            width=8,
            fg="#000000",
            bg="#FFFFFF",
            text=avgSpotifyPopularityNumber,
            anchor="e",
            padx=5,
            pady=5,
        )
        ...
        analyticsDataForGraphs = pd.DataFrame(
            analyticsDataForGraphs,
            columns=[
                "song title",
                "youtube views",
        ...
        analyticsDataForScatterPlot = analyticsDataForGraphs.drop(
            ["pop", "hiphop", "indie", "kpop", "explicit", "inhouse", "lofi"], axis=1
        )
        ...
        def onCanvasClick(event):
            ...
            scatter = ax.scatter(
                dataFrameScatterPlot["youtube views"],
                dataFrameScatterPlot["spotify popularity"],
                c=factorizeColor,
                cmap=cm.viridis,
                label=dataFrameScatterPlot["song title"],
            )
            ...
            fullScreenScatter.bind(
                "<Escape>", lambda event: fullScreenScatter.destroy()
            )
        ...
        for m in range(len(analyticsDataForGraphs)):
            ...
            if popValueForAnalytics != "None":
                popValueForAnalyticsTotalYoutube = int(
                    analyticsDataForGraphs.loc[m, "youtube views"].replace(",", "")
                ) + int(popValueForAnalyticsTotalYoutube)
                popValueForAnalyticsTotalSpotify = int(
                    analyticsDataForGraphs.loc[m, "spotify popularity"]
                ) + int(popValueForAnalyticsTotalSpotify)
                errorStacked = False

            if hiphopValueForAnalytics != "None":
                hiphopValueForAnalyticsTotalYoutube = int(
                    analyticsDataForGraphs.loc[m, "youtube views"].replace(",", "")
                ) + int(hiphopValueForAnalyticsTotalYoutube)
                hiphopValueForAnalyticsTotalSpotify = int(
                    analyticsDataForGraphs.loc[m, "spotify popularity"]
                ) + int(hiphopValueForAnalyticsTotalSpotify)
                errorStacked = False

            if indieValueForAnalytics != "None":
                indieValueForAnalyticsTotalYoutube = int(
                    analyticsDataForGraphs.loc[m, "youtube views"].replace(",", "")
                ) + int(indieValueForAnalyticsTotalYoutube)
                indieValueForAnalyticsTotalSpotify = int(
                    analyticsDataForGraphs.loc[m, "spotify popularity"]
                ) + int(indieValueForAnalyticsTotalSpotify)
                errorStacked = False

            if kpopValueForAnalytics != "None":
                kpopValueForAnalyticsTotalYoutube = int(
                    analyticsDataForGraphs.loc[m, "youtube views"].replace(",", "")
                ) + int(kpopValueForAnalyticsTotalYoutube)
                kpopValueForAnalyticsTotalSpotify = int(
                    analyticsDataForGraphs.loc[m, "spotify popularity"]
                ) + int(kpopValueForAnalyticsTotalSpotify)
                errorStacked = False

            if explicitValueForAnalytics != "None":
                explicitValueForAnalyticsTotalYoutube = int(
                    analyticsDataForGraphs.loc[m, "youtube views"].replace(",", "")
                ) + int(explicitValueForAnalyticsTotalYoutube)
                explicitValueForAnalyticsTotalSpotify = int(
                    analyticsDataForGraphs.loc[m, "spotify popularity"]
                ) + int(explicitValueForAnalyticsTotalSpotify)
                errorStacked = False

            if inhouseValueForAnalytics != "None":
                inhouseValueForAnalyticsTotalYoutube = int(
                    analyticsDataForGraphs.loc[m, "youtube views"].replace(",", "")
                ) + int(inhouseValueForAnalyticsTotalYoutube)
                inhouseValueForAnalyticsTotalSpotify = int(
                    analyticsDataForGraphs.loc[m, "spotify popularity"]
                ) + int(inhouseValueForAnalyticsTotalSpotify)
                errorStacked = False

            if lofiValueForAnalytics != "None":
                lofiValueForAnalyticsTotalYoutube = int(
                    analyticsDataForGraphs.loc[m, "youtube views"].replace(",", "")
                ) + int(lofiValueForAnalyticsTotalYoutube)
                lofiValueForAnalyticsTotalSpotify = int(
                    analyticsDataForGraphs.loc[m, "spotify popularity"]
                ) + int(lofiValueForAnalyticsTotalSpotify)
                errorStacked = False
        ...
        if errorStacked == False:
            stackedPlotData.append(
                [
                    popValueForAnalyticsTotalYoutube,
                    hiphopValueForAnalyticsTotalYoutube,
                    indieValueForAnalyticsTotalYoutube,
                    kpopValueForAnalyticsTotalYoutube,
                    explicitValueForAnalyticsTotalYoutube,
                    inhouseValueForAnalyticsTotalYoutube,
                    lofiValueForAnalyticsTotalYoutube,
                ]
            )
            stackedPlotData.append(
                [
                    popValueForAnalyticsTotalSpotify,
                    hiphopValueForAnalyticsTotalSpotify,
                    indieValueForAnalyticsTotalSpotify,
                    kpopValueForAnalyticsTotalSpotify,
                    explicitValueForAnalyticsTotalSpotify,
                    inhouseValueForAnalyticsTotalSpotify,
                    lofiValueForAnalyticsTotalSpotify,
                ]
            )
            dataFrameStackedPlot = pd.DataFrame(
                {
                    "tags": [
                        "pop",
                        "hiphop",
                    ...
                    "youtube views": stackedPlotData[0],
                    "spotify popularity": stackedPlotData[1],
                }
            )
        ...
        def onCanvasClick2(event):
            ...
            ax.bar(
                dataFrameStackedPlot["tags"],
                dataFrameStackedPlot["youtube views"],
                color=cmap(dataFrameStackedPlot["spotify popularity"]),
            )
            ax.set_xlabel("tags")
            ...
            fullScreenStacked.bind(
                "<Escape>", lambda event: fullScreenStacked.destroy()
            )
...
def showEntryPage(recievedData):
    ...
    (
        ...
        indieVarTempValue,
        kpopVarTempValue,
        explicitVarTempValue,
        inhouseVarTempValue,
        lofiVarTempValue,
        ArtworkImageLocationTempValue,
        importSongTempValue,
        addMiscFilesTempValue,
        importArtworkImageTempValue,
        youtubeLinkTemp,
        spotifyLinkTemp,
    ) = fetchAllEntries[0]
    maxLengthOfTextTemp = 45
    songShortenedLocationTemp = (
        importSongTempValue[:maxLengthOfTextTemp] + "..."
        if len(importSongTempValue) > maxLengthOfTextTemp
        else importSongTempValue
    )
    maxLengthOfMiscTextTemp = 20
    addMiscFilesTempValueShortened = (
        addMiscFilesTempValue[:maxLengthOfMiscTextTemp] + "..."
        if len(addMiscFilesTempValue) > maxLengthOfMiscTextTemp
        else addMiscFilesTempValue
    )
    ...
    def saveFile(initialDirectory):
        if os.path.exists(initialDirectory):
            ...
            messagebox.showwarning(
                "internal error",
                "it seems that the original file has been deleted from the databasefiles folder. check your recycle bin or cloud saves to retrieve it",
            )

    tagsTemp.grid(row=1, column=0, sticky="w", pady=(15, 0))
    importsGridTemp = tk.Label(
        splitGridTemp, bg="#FFFFFF", borderwidth=0, relief="flat", justify="left"
    )
    ##
    importArtworkImageTemp = tk.Label(
        importsGridTemp,
        image=newPhotoTemp,
        anchor="c",
        bg="#FFFFFF",
    ...
    importsGridInsideTemp = tk.Label(
        importsGridTemp, bg="#FFFFFF", borderwidth=0, relief="flat", justify="right"
    )
    ##
    importSongTemp = tk.Label(
        importsGridInsideTemp,
        text=songShortenedLocationTemp,
        font='"Space Grotesk" 13',
        anchor="sw",
        bg="#FFFFFF",
        fg="#B0B0B0",
    ...
    ##
    addMiscFilesTemp = tk.Button(
        importsGridInsideTemp,
        text=addMiscFilesTempValueShortened,
        borderwidth=1,
        relief="solid",
        font='"Space Grotesk" 13',
        width=21,
        ...
        anchor="w",
        cursor="hand2",
        command=lambda: webbrowser.open(addMiscFilesTempValue),
    )
    ...
    downloadArtworkButton = tk.Button(
        ...
        cursor="hand2",
        justify=tk.LEFT,
        command=lambda: saveFile(ArtworkImageLocationTempValue),
    )
    ...
    downloadSongButton = tk.Button(
        ...
        cursor="hand2",
        justify=tk.LEFT,
        command=lambda: saveFile(importSongTempValue),
    )
    ...
    performedByTemp = tk.Label(
        splitGridTemp,
        text=performedByTempValue,
        borderwidth=1,
        relief="solid",
        font='"Space Grotesk" 13',
        width=38,
    ...
    writtenByTemp = tk.Label(
        splitGridTemp,
        text=writtenByTempValue,
        borderwidth=1,
        relief="solid",
        font='"Space Grotesk" 13',
        width=38,

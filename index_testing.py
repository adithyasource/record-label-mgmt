...
def addValuesToDB():
    ...
    def deleteEntryWithAnalytics(recievedData):
        ...
        if deleteReleaseQuestion == 'yes':
            ...
            conn.commit()
            conn.close()
            # entry.destroy()  # delete the UI element
...
saveRelease = tk.Button(splitGrid, font='"Space Grotesk" 13', bg='#FFFFFF', text='save release', relief='flat', activebackground='#FFFFFF', command=lambda: [doShit(frame), convertImageIntoBinary(photo), addValuesToDB()], borderwidth=0, cursor='hand2')
